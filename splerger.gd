...
func split_by_surface(orig_mi : MeshInstance3D, attachment_node : Node, use_local_space : bool = false):
	...
	for s in range (count):
		...
		newstuff.append(_split_mesh_by_surface(mdt, orig_mi, attachment_node, s, use_local_space))
...
func merge_suitable_meshes_across_branches(root : Node3D):
	...
	for n in range (sub_list.size()):
		...
		if (sl.size() > 1):
			var new_mi : MeshInstance3D = merge_MeshInstance3Ds(sl, root)
...
func _merge_suitable_child_meshes(node : Node):
	if node is Node3D:
		...
		if (child_list.size() > 1):
			var new_mi : MeshInstance3D = merge_MeshInstance3Ds(child_list, node)
...
func merge_MeshInstance3Ds(mesh_array, attachment_node : Node, use_local_space : bool = false, delete_originals : bool = true):
	if mesh_array.size() < 2:
		printerr("merge_MeshInstance3Ds array must contain at least 2 meshes")
		return
	...
	if first_mi is MeshInstance3D:
		...
		printerr("merge_MeshInstance3Ds array must contain mesh instances")
		return
	...
	for n in range (mesh_array.size()):
		vertex_count = _merge_MeshInstance3D(st, mesh_array[n], use_local_space, vertex_count)
...
func _merge_MeshInstance3D(st : SurfaceTool, mi : MeshInstance3D, use_local_space : bool, vertex_count : int):
	if mi == null:
		printerr("_merge_MeshInstance3D - not a mesh instance, ignoring")
		return vertex_count

	print("merging MeshInstance3D : " + mi.get_name())		
	var mesh = mi.mesh
